...
/**
 * Privacy Policy — public legal page (§27).
 *
 * Phase 14: content is now DB-driven from `/api/site-content` keyed
 * by `(privacy_policy.body, locale)`. The backend defaults to the
...
export function PrivacyPolicy() {
  ...
  useEffect(() => {
    let cancelled = false;
    setRow('loading');
    getSiteContent('privacy_policy.body', locale.code)
      .then((r) => {
  ...
  return (
    <>
      <section className="wrap" style={{ paddingTop: 56, paddingBottom: 0 }}>
        ...
        >
          {locale.code === 'ar' ? 'سياسة الخصوصية' : 'Privacy Policy'}
        </h1>
        <p
          className="hero-sub"
        ...
        >
          {locale.code === 'ar'
            ? 'كيف تجمع D-Trips بياناتك وتستخدمها وتحميها.'
            : 'How D-Trips collects, uses, and protects the information you share with us.'}
        </p>
        {showDraftBanner && (
          <div className="legal-banner" role="status">
            <strong>DRAFT.</strong> {locale.code === 'ar'
              ...
              : 'This page is placeholder content. The published Privacy Policy will be supplied by the business\'s legal counsel and rendered from the backend SiteContent table.'}
          </div>
        )}
      </section>
